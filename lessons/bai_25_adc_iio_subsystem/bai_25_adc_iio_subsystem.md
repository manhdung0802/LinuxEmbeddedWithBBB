# Bài 25 - ADC Driver (IIO Subsystem)

## 1. Mục tiêu bài học
- Hiểu IIO (Industrial I/O) subsystem chi tiết
- AM335x TSC_ADC subsystem (Touchscreen ADC)
- `iio_dev`, `iio_chan_spec`, `iio_info`
- Viết ADC driver tích hợp IIO framework
...
AM335x có module TSC_ADC kết hợp Touchscreen controller và ADC:
...
### 2.2 Chân ADC trên BBB

| Channel | Pin Header | Tên |
|---------|-----------|------|
...
```
1. Cấu hình step (channel, mode, averaging)
2. Enable step → hardware tự chuyển đổi
3. Kết quả vào FIFO
4. Software đọc từ FIFO

┌──────────┐    ┌──────────┐    ┌──────┐
│ AINx pin │───►│Step Logic│───►│ FIFO │───► Software đọc
└──────────┘    └──────────┘    └──────┘
                     ▲
              STEPCONFIG[n]
              (channel, avg)
...
| Return | Ý nghĩa | Sysfs giá trị |
|--------|---------|----------------|
| IIO_VAL_INT | Số nguyên | val |
...
# Tính voltage: 2048 * 0.439453125 = 900 mV
```

---

## 6. IIO Buffered Mode (nâng cao)

Để đọc data liên tục, IIO hỗ trợ buffered mode:

```c
/* Khai báo buffer support */
...
static irqreturn_t am335x_adc_trigger_handler(int irq, void *p)
{
    ...
    /* Đọc all channels, push to buffer */
...
5. **Step-based**: mỗi step config: channel, averaging, delay
6. **Buffered mode**: continuous data với triggered_buffer

---

## 8. Câu hỏi kiểm tra

1. AM335x ADC có bao nhiêu channel? Range input là bao nhiêu?
2. `IIO_VAL_FRACTIONAL_LOG2` với val=1800, val2=12 trả về gì?
3. FIFO0DATA register chứa gì?
4. Tại sao phải disable ADC trước khi thay đổi STEPCONFIG?
5. Sự khác nhau giữa direct mode và buffered mode trong IIO?

---

## 9. Chuẩn bị cho bài sau

Bài tiếp theo: **Bài 26 - Timer Driver**

Ta sẽ học clocksource, clockevent, hrtimer và AM335x DMTIMER.
