# Bài 22 - SPI Driver thực hành

## 1. Mục tiêu bài học
- Viết driver hoàn chỉnh cho MCP3008 ADC qua SPI
- Tích hợp IIO subsystem cho ADC readings
- Hiểu SPI protocol của MCP3008
- Device Tree + pinmux cho SPI0 trên BBB
...
static int mcp3008_read_channel(struct mcp3008_data *adc, int channel)
{
	...
	/* Tạo command: Start bit + single-ended + channel */
...
1. MCP3008 cần 3 byte SPI transaction. Giải thích format từng byte TX/RX.
2. Tại sao dùng 1 `spi_transfer` với len=3 thay vì 3 transfer riêng của 1 byte?
3. `IIO_VAL_FRACTIONAL_LOG2` trả về giá trị scale ra sao?
4. Tại sao cần `__aligned(IIO_DMA_MINALIGN)` cho SPI buffer?
5. Làm sao thêm differential channel cho MCP3008?

---

## 9. Chuẩn bị cho bài sau

Bài tiếp theo: **Bài 23 - UART/Serial Driver**

Ta sẽ học serial core framework: `uart_driver`, `uart_port`, `uart_ops` và AM335x UART registers.
