# B�i 25 - AM335x UART & Serial Driver

> **Tham chi?u ph?n c?ng BBB:** Xem [uart.md](../mapping/uart.md)

## 0. BBB Connection � UART tr�n BeagleBone Black

> :warning: **AN TOAN DIEN:**
> - UART tren BBB la **3.3V logic (TTL)** - KHONG NOI RS-232 (+/-12V) truc tiep!
> - Dung **USB-UART converter 3.3V** (CP2102/CH340/FT232RL) - kiem tra jumper 3.3V tren module
> - Noi TX->RX, RX->TX (cheo) + GND chung

### 0.1 So d? k?t n?i th?c t? (UART1 � khuy�n d�ng cho h?c)

```
BeagleBone Black P9 header
                        +-------------------------+
  P9.24 (UART1_TXD) --?�  USB-UART Converter     �--? PC /dev/ttyUSB0
  P9.26 (UART1_RXD) ?--�  (CP2102/CH340/FT232)   �?-- PC
  P9.1/P9.2  (GND) ----�                         �
                        +-------------------------+

  Ho?c n?i th?ng 2 BBB v?i nhau:
  BBB1 P9.24 (TX) --?---- P9.26 (RX) BBB2
  BBB1 P9.26 (RX) --?---- P9.24 (TX) BBB2
  BBB1 P9.1  (GND) ------- P9.1 (GND) BBB2
```

### 0.2 B?ng UART ? BBB Pin

| Module | Base Address | Device Node | TX Pin | RX Pin | M?c d�ch BBB |
|--------|-------------|------------|--------|--------|-------------|
| UART0 | 0x44E09000 | /dev/ttyO0 | J1.5 | J1.6 | **Debug console** � boot messages |
| **UART1** | **0x48022000** | **/dev/ttyO1** | **P9.24** | **P9.26** | **D�NG CHO H?C** |
| UART2 | 0x48024000 | /dev/ttyO2 | P9.21 | P9.22 | Xung d?t SPI0 |
| UART4 | 0x481A8000 | /dev/ttyO4 | P9.13 | P9.11 | D�ng du?c |
| UART5 | 0x481AA000 | /dev/ttyO5 | P8.37 | P8.38 | D�ng du?c |

### 0.3 Pinmux UART1 (P9.24=TX, P9.26=RX)

```dts
uart1_pins: uart1_pins {
    pinctrl-single,pins = <
        /* P9.24 = uart1_txd, ball D15, mode 0 */
        AM33XX_PADCONF(AM335X_PIN_UART1_TXD, PIN_OUTPUT, MUX_MODE0)
        /* P9.26 = uart1_rxd, ball D16, mode 0 */
        AM33XX_PADCONF(AM335X_PIN_UART1_RXD, PIN_INPUT_PULLUP, MUX_MODE0)
    >;
};
```

### 0.4 Test UART1 tr�n BBB

```bash
# Tr�n BBB � c?u h�nh UART1 + test loopback
stty -F /dev/ttyO1 115200 cs8 -cstopb -parenb
echo "Hello from BBB UART1" > /dev/ttyO1
cat /dev/ttyO1   # d?c d? li?u nh?n v?
```

> **Ngu?n**: BeagleboneBlackP9HeaderTable.pdf (P9.24/P9.26); spruh73q.pdf Chapter 19 (UART)

---

## 1. M?c ti�u b�i h?c
- Hi?u Serial Core framework trong Linux kernel
- Ki?n tr�c `uart_driver`, `uart_port`, `uart_ops`
- AM335x UART1 (0x48022000) registers � d�ng P9.24/P9.26 tr�n BBB
- Vi?t UART platform driver cho UART1 BBB
...
AM335x c� 6 UART module:

| Module | Base Address | BBB usage | Ngu?n: spruh73q.pdf |
|--------|-------------|-----------|---------------------|
...
### 2.2 Thanh ghi quan tr?ng (16550 compatible)
...
Userspace write("Hello")
  ...
  uart_ops->start_tx()
       �
       ?
  �?c t? circ_buf ? ghi v�o THR register
       �
       ?
...
1. **Serial Core** 3 t?ng: `uart_driver` ? `uart_port` ? `uart_ops`
2. **`uart_register_driver()`** ph?i g?i tru?c `uart_add_one_port()`
3. **Baud rate divisor**: `divisor = uartclk / (16 � baud)`
4. **LCR[7]=1** d? access DLL/DLH (divisor latch)
5. **IRQ handler**: d?c LSR ki?m DR (RX) v� THRE (TX)
6. **AM335x UART** l� 16550-compatible v?i FIFO 64 byte
7. **`uart_insert_char()` + `tty_flip_buffer_push()`** d?y RX data l�n TTY layer

---

## 8. C�u h?i ki?m tra

1. T?i sao ph?i g?i `uart_register_driver()` tru?c `uart_add_one_port()`?
2. Gi?i th�ch c�ch t�nh baud rate divisor cho 115200 baud v?i clock 48MHz.
3. LSR register bit n�o cho bi?t c� th? ghi THR? Bit n�o cho bi?t c� data RX?
4. `tty_flip_buffer_push()` l�m g�?
5. T?i sao `learn_uart_init()` kh�ng d�ng `module_platform_driver()` macro?

---

## 9. Chu?n b? cho b�i sau

B�i ti?p theo: **B�i 24 - PWM Subsystem**

Ta s? h?c PWM framework: `pwm_chip`, `pwm_ops`, v� EHRPWM tr�n AM335x.
