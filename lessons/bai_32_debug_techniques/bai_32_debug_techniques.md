...
`pr_debug()` và `dev_dbg()` m?c d?nh b? t?t. Dynamic debug cho phép b?t/t?t t?i runtime.

### 3.2 S? d?ng

```bash
# Mount debugfs (thu?ng t? mount)
mount -t debugfs none /sys/kernel/debug

# Xem t?t c? debug statements
cat /sys/kernel/debug/dynamic_debug/control

# B?t debug cho module c? th?
echo 'module my_driver +p' > /sys/kernel/debug/dynamic_debug/control

# B?t debug cho file c? th?
echo 'file my_driver.c +p' > /sys/kernel/debug/dynamic_debug/control

# B?t debug cho function
echo 'func my_probe +p' > /sys/kernel/debug/dynamic_debug/control

# B?t debug + function name + line number
...
### 5.1 T?o debugfs entries trong driver
...
2. **Dynamic debug**: b?t/t?t `pr_debug()` t?i runtime qua debugfs
3. **ftrace**: function tracer, function_graph, event tracing
4. **debugfs**: t?o file debug trong `/sys/kernel/debug/`
5. **i2c-tools / devmem2**: debug hardware register tr?c ti?p
6. **OOPS decode**: `addr2line` + `objdump` xác d?nh dòng code gây crash

---

## 9. Câu h?i ki?m tra

1. `dev_dbg()` khác `dev_info()` th? nào? Khi nào `dev_dbg()` xu?t hi?n trong dmesg?
2. Cách b?t dynamic debug cho t?t c? hàm trong module "my_driver"?
3. ftrace function_graph tracer hi?n th? gì?
4. `debugfs_create_x32()` t?o file debugfs d?ng gì?
5. Ð?c kernel OOPS: "PC is at my_probe+0x3c" nghia gì?

---

## 10. Chu?n b? cho bài sau

Bài ti?p theo: **Bài 32 - Capstone Project**

Ta s? tích h?p t?t c? ki?n th?c: GPIO + I2C + interrupt + Device Tree + sysfs + debugfs.
