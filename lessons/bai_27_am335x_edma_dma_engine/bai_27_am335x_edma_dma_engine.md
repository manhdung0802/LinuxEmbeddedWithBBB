# B�i 27 - AM335x EDMA & DMA Engine

> **Tham chi?u ph?n c?ng BBB:** Xem [edma.md](../mapping/edma.md)

## 0. BBB Connection � EDMA tr�n BBB

```
AM335x EDMA Controller
----------------------
 EDMA CC @ 0x49000000, IRQ 12 (completion), IRQ 14 (error)
 EDMA TC0 @ 0x49800000
 EDMA TC1 @ 0x49900000
 EDMA TC2 @ 0x49A00000

 64 DMA channels, 256 PaRAM sets

  K�nh DMA thu?ng d�ng tr�n BBB:
  ---------------------------------
  Channel 24,25: McSPI0 TX/RX (P9.17-22)
  Channel 26,27: McSPI1 TX/RX
  Channel 18,19: I2C2 TX/RX (P9.19-20)
  Channel 8,9:   UART1 TX/RX (P9.24-26)
  Channel 56-63: ADC (TSCADC FIFO)

  ?ng d?ng: DMA cho ADC continuous sampling,
           SPI bulk transfer, memory-to-memory copy
```

---

## 1. M?c ti�u b�i h?c
- Hi?u DMA Engine framework trong Linux kernel
- AM335x EDMA (Enhanced DMA) controller
- `dmaengine` API: `dma_request_chan()`, `dmaengine_prep_slave_sg()`
- Vi?t driver s? d?ng DMA transfer
...
M?i DMA channel c� m?t PaRAM set ch?a th�ng tin transfer:
...
chan = dma_request_chan(&pdev->dev, "rx");  /* T�n t? DT */
...
/* 5. Issue pending - b?t d?u transfer */
dma_async_issue_pending(chan);

/* 6. Ch? ho�n th�nh (ho?c d�ng callback) */
...
static int learn_dma_transfer(struct learn_dma *dma, struct device *dev)
{
	...
	/* Ch? DMA ho�n th�nh, timeout 5 gi�y */
...
```c
/* V� d?: DMA t? SPI RX register v�o memory buffer */
struct dma_slave_config cfg = {
    .direction = DMA_DEV_TO_MEM,
...
3. **`dma_alloc_coherent()`**: c?p ph�t DMA-able memory (uncached)
4. **Memcpy vs Slave**: memcpy = mem-to-mem, slave = peripheral-to-mem ho?c ngu?c l?i
5. **EDMA PaRAM**: src, dst, count, link ? hardware t? transfer
6. **Callback**: ch?y trong interrupt context ? kh�ng sleep

---

## 8. C�u h?i ki?m tra

1. `dma_alloc_coherent()` kh�c g� `kmalloc()`?
2. Ph�n bi?t `dmaengine_prep_dma_memcpy()` v� `dmaengine_prep_slave_sg()`.
3. T?i sao ph?i g?i `dma_async_issue_pending()` sau `dmaengine_submit()`?
4. DMA callback ch?y trong context g�? C� du?c g?i `mutex_lock()` trong d� kh�ng?
5. EDMA PaRAM set ch?a nh?ng g�?

---

## 9. Chu?n b? cho b�i sau

B�i ti?p theo: **B�i 28 - Power Management**

Ta s? h?c Runtime PM, PM ops, suspend/resume, clock gating tr�n AM335x.
